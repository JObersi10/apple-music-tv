...
class DevMenuViewModel @Inject constructor(
    ...
    fun refresh() = viewModelScope.launch {
        ...
        runCatching { repo.getAuthStatus() }.onSuccess { s ->
            ...
            _state.update { it.copy(serverOk = true, hasBearer = s.hasBearer, hasMUT = localHasMUT, mutSetAt = s.mutSetAt) }
            ...
            _state.update { s -> s.copy(serverOk = false) }
            log("ERROR", "Server unreachable: ${it.message}")
        }
    }

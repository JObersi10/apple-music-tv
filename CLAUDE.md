...
- **Search "More to Explore"**: pill links (Charts / Browse by Genre / Moods & Activities / Decades) → editorial rooms via the `"room"` curator kind. `browse/room/:id` prefixes apple-curator/curator ids (`ac-`/`c-`) and `CategoryScreen` routes those tiles into nested category pages (`onCuratorClick`).

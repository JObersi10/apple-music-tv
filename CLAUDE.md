...
- **Multi-segment HLS**: some tracks use fMP4 HLS with init-segment + multiple audio segments. `stream_decrypt.py:fetch_encrypted` downloads ALL segments in parallel (`asyncio.gather`) and concatenates them before decryption. Grabbing only `#EXT-X-MAP` (init) causes choppy/silent audio. ffmpeg uses `-c copy` for single-segment files; for multi-segment it uses `-af aresample=async=1` to fix timestamp gaps at segment boundaries (those gaps cause `UnexpectedDiscontinuityException` in ExoPlayer). Re-encode is fast for AAC/ctrp64 (which we always prefer).
...
- **Color extraction**: `rememberArtworkPalette` loads artwork via Coil at 1200px, runs `Palette.from()`, picks up to 6 distinct swatches (vibrant → dark vibrant → muted → light vibrant → dark muted → light muted → dominant). Bright colors are luminance-capped (any swatch with luma > 0.55 gets scaled down) so nothing blows out. Falls back to a dark blue/purple palette if artwork unavailable.
- **Blobs**: 4 radial gradient blobs, one draw call each, `BlendMode.Screen` — where blobs overlap, colors add/brighten creating an iridescent oil-on-water mix. Colors cross-fade over 1.5s on song change. If palette has fewer than 4 colors, they cycle. Blob radius `0.62 × max(w,h)` (large enough to cover screen with 4 blobs). **Do not exceed 4 blobs** — Screen blend forces an offscreen compositing pass per draw call; 6 blobs caused lag on Fire TV. No hardware blur (no-op on Fire TV API < 31, not needed).
- **Motion**: 3 `InfiniteTransition` floats (t1: 20s, t2: 27s, t3: 34s), `LinearEasing`, `RepeatMode.Reverse`. Each blob center uses a different combination of t1/t2/t3 axes so all 6 move independently. Centers are biased toward screen edges so blobs don't converge in the middle.
- **Center darkening**: a single `radialGradient` draw (black 33%→0%) at screen center suppresses the white hotspot that forms when all blobs overlap centrally.
- **Beat reactivity**: `BeatAnalyzer : BaseAudioProcessor` injected into ExoPlayer's `DefaultAudioSink` via `BeatAwareRenderersFactory`. Computes RMS energy per PCM_16BIT buffer → `StateFlow<Float>` (0..1). Collected in Compose, smoothed with `animateFloatAsState(tween(80))`. Scales blob radius by `1 + energy*0.18` and alpha by `+0.12`. Subtle pulse — not a hard thump. Only active for PCM_16BIT; bypassed for float output.
- **No vignette** — removed; colors show through fully.
- **Perf budget**: 5 draw calls per frame (4 blobs + 1 center darken). 3 drift animators. Color extraction on `Dispatchers.IO`. Do NOT increase beyond 4 blobs, add `Modifier.blur()`, multi-pass draws, or `CubicBezierEasing` — all caused lag on Fire TV.

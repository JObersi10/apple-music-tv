...
- **Standalone (on-device) playback**: `StandalonePreferences` (`standalone_prefs`, StateFlow), toggled on the :8080 page. `useStandalone() = standalonePrefs.isEnabled() || !serverPrefs.serverReachable`. **It's a speed feature, not just a fallback** — the proxy can't serve a byte until the whole track is downloaded, mp4decrypted and remuxed (15-20s cold), while ExoPlayer decrypts HLS segments as it plays (~1s start). `buildStandaloneSource(song)` is **suspend** (bearer scrape + webPlayback + Widevine `DefaultDrmSessionManager`); callers must await it before `play()` — firing it and falling through meant the *previous* track kept playing and faded back in. `buildCrossfadeExo()` has no DRM of its own, so the crossfade partner needs its own standalone source too, or every transition degrades to a hard cut with the server off. Prefetch is skipped in standalone (nothing to warm). Falls back to the proxy URL when the source can't be built. `usingStandalone` is currently a write-only field — dead.

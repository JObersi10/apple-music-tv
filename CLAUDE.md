...
- **Blobs**: 6 radial gradient blobs, one draw call each, `BlendMode.Screen` — where blobs overlap, colors add/brighten creating an iridescent oil-on-water mix. Colors cross-fade over 1.5s on song change. If palette has fewer than 6 colors, they cycle. Blob radius `0.52 × max(w,h)`. No hardware blur (no-op on Fire TV API < 31, and not needed).
...
- **Perf budget**: 7 draw calls per frame (6 blobs + 1 center darken). 3 drift animators. Color extraction on `Dispatchers.IO`. Do NOT add `Modifier.blur()`, multi-pass draws, or `CubicBezierEasing` — all caused lag on Fire TV.

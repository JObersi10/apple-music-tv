    ...
    try {
      ...
      for (const rec of recs) {
        ...
        if (recType === "stations" || title.toLowerCase().includes("station")) {
          stationsForYou.push(...items);
        } else if (title.toLowerCase().includes("genre") || rec.attributes?.kind === "genre-mix") {
          genreSections.set(title, items);
        } else {
          // Mix all non-station recs into Top Picks row
          topPicks.push(...items);
        }
      }

      if (topPicks.length > 0) sections.push({ title: "Top Picks for You", albums: topPicks });
      if (stationsForYou.length > 0) sections.push({ title: "Stations for You", albums: stationsForYou });
  ...
  if (mut) {
    try {
      const res = await axios.get(`${APPLE}/v1/me/recent/played`, {
        params: { limit: 20, types: "albums,playlists,stations" },
        headers: h,
      });
